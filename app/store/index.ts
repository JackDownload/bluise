...
export const actions: Actions<State, State> = {
  async GET_POSTS_LIST({ commit }): Promise<void | Error> {
    ...
    const categores = await getContent({ context, prefix: 'recipe' });
  ...
  async nuxtServerInit({ dispatch }): Promise<void> {
    await Promise.all([dispatch('GET_PAGES_LIST'), dispatch('GET_POSTS_LIST'), dispatch('GET_RECIPES_LIST'), dispatch('GET_CATEGORIES_LIST')]);
  },
};
